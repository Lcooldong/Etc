...
int cmd_direction(int argc, char *argv[]);
...
tCmdLineEntry g_psCmdTable[] =
{
    ...
    { "on",       cmd_on,       "LED ON" },		
	  ...
	  { "tim3_period",      cmd_tim3_period,      "Tim3 Preiod" },
		...
		{ "direction",  cmd_direction, "DC Motor direction"},
		{ "servo", cmd_servo, "Servo Angle" },
    { 0, 0, 0 }
};
...
int cmd_on(int argc, char *argv[]) {
  ...
  GPIOF->ODR =  GPIOF->ODR  | (1 << led_num);	//set
...
int cmd_off(int argc, char *argv[]) {
  ...
  GPIOF->ODR =  GPIOF->ODR  & ~(1 << led_num);	//reset
...
int cmd_servo(int argc, char *argv[]) {
  ...
  uint16_t angle = 0;
	...
	if(argc != 2 )
  {
     printf(" servo [duty ratio]\n\r");
     return 0;
  }
	
	angle = (uint16_t)(atoul(argv[1]));
	
	duty_num = (__HAL_TIM_GET_AUTORELOAD(&htim5) * duty_num )/ 1000;
	printf("PWM CH2, Period[%d], Compare Value[%d] Angle[%d] \n\r", __HAL_TIM_GET_AUTORELOAD(&htim5), duty_num, angle);
...
int cmd_direction(int argc, char *argv[]){
	...
	if(argc != 2)	// input count
	{
		printf(" direction [0 or 1]\n\r");
		return 0;
	}
	
	dir = (uint16_t)(atoul(argv[1]));
